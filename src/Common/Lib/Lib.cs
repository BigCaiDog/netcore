...
public static class Lib {
    ...
    private static int[] specialChars = { 34, 39, 92, 8216, 8220, 8217 };
    /// <summary>
    /// 特殊字符过滤FOR SQL
    /// </summary>
    /// <param name="srcData"></param>
    /// <returns></returns>
    public static string specialCharFilter(this string srcData)
    {
        if (string.IsNullOrWhiteSpace(srcData))
            return null;
        char[] srcDataArray = srcData.ToCharArray();
        srcDataArray = srcDataArray.Where(a => !specialChars.Contains(a)).ToArray();
        srcData = string.Join("", srcDataArray);
    ...
    private static readonly DateTime dataTimeStart = new DateTime(1970, 1 , 1, 8, 0 , 0);
    public static long ToTimeStamp(this DateTime srcTime)
    {
        return (long)(srcTime - dataTimeStart).TotalMilliseconds;
    }
    ...
    /// <returns></returns>
    public static T GetJObject<T>(this string str)
    {
